...
const binaryNumberInput = document.querySelector(
  "[data-input=number-binary]"
) as HTMLInputElement;
const decimalNumberInput = document.querySelector(
  "[data-input=number-decimal"
) as HTMLInputElement;
const containerErrorBinary = document.querySelector(
  "[data-erro=error-binary"
) as HTMLDivElement;

// eslint-disable-next-line max-len
const checkIfBinaryIsValid = (numberBinary: string, regex: RegExp): boolean =>
  !!new RegExp(regex).exec(numberBinary);

const convertBinaryToDecimal = (numberBinary: string): number | string =>
  parseInt(numberBinary, 2) || "";
...
const showDecimalNumber = (
  ...
  decimalNumberInput.value = !verifiedValue
    ? decimalNumber.toString()
    : "binary number invalid";
};
...
const handleInput = (event: Event): void => {
  ...
  const verifiedValue = checkIfBinaryIsValid(
    binaryInputValue,
    ExpCheckDifferentNumber
  );
